    ...
    def BuyOrSell(self,NowPrice,row):
        ...
        if NowPrice - float(row[2]) > 4:   #### should be bottom 
            if self.BuyInformation[0]==0:
                ...
                self.resultcommand.execute(("INSERT INTO Futures VALUES (?,?)","做多新倉",NowPrice))
            ...
            if self.BuyInformation[0]==0:
                ...
                self.Record.append(NowPrice)
                self.resultcommand.execute(("INSERT INTO Futures VALUES (?,?)","做空新倉",NowPrice))
        ################################################################################
        ...
        if self.BuyInformation[0]==1:
            ...
            if self.BuyInformation[2]=="Positive":
                if float(NowPrice) - float(self.BuyInformation[1]) > 15:
                    ...
                    print("做多平倉")
                    self.resultcommand.execute(("INSERT INTO Futures VALUES (?,?)","做多平倉",NowPrice))
            ...
            if self.BuyInformation[2]=="Negative":
                if float(NowPrice) - float(self.BuyInformation[1]) < -15:
                    ...
                    self.ResetInfromation()
                    print("做空平倉")
                    self.resultcommand.execute(("INSERT INTO Futures VALUES (?,?)","做空平倉",NowPrice))
